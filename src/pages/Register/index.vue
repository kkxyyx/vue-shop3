  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <span class="error-msg">{{ errors.first("code") }}</span>
        <button style="width: 100px; height: 36px" @click="getCode()">
          发送验证码
        </button>
      ...
      <div class="content">
        <label>登录密码:</label>
      <input placeholder="请输入你的登录密码" v-model="password" name="password" v-validate="{ required: true, regex: /^[a-zA-Z]\w{5,17}$/ }"
...
export default {
  ...
  methods: {
    ...
    // 用户注册
    async userRegister() {
      // const success = await this.$validator.validateAll(); //全部表单验证成功返回true
      if (true) {
        // 表单验证成功，再向服务器发请求
        try {
