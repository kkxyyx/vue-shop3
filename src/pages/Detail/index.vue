            ...
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  ...
                  v-for="(list, index) in item.spuSaleAttrValueList"
                  :key="index"
                  @click="changeActive(list, item.spuSaleAttrValueList)"
                >
  ...
  methods: {
    ...
    async addShopcar() {
      try {
        ...
        // 复杂数据用 本地存储|会话存储，一般存储的是字符串
        localStorage.setItem("SKUINFO", JSON.stringify(this.skuInfo));
        // 加入购物车成功，路由跳转
        this.$router.push({
          name: "addcartsuccess",
          query: { skuNum: this.skuNum }, // 简单数据直接传
        });
      } catch (error) {
        alert(error.message);
      }
